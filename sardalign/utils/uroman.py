...
def robust_str_to_num(
    num_s: str, filename: str = None, line_number: Optional[int] = None, silent: bool = False
...
class Uroman:
            ...
            for line_number, line in enumerate(f, 1):
                ...
                if script_name := slot_value_in_double_colon_del_list(line, "script-name"):
                    ...
                    if lc_script_name in self.scripts:
                        sys.stderr.write(
                            f'** Ignoring duplicate script "{script_name}" ' f"in line {line_number} of {filename}\n"
                        )
                    else:
                        n_entries += 1
                        direction = slot_value_in_double_colon_del_list(line, "direction")
                        abugida_default_vowel_s = slot_value_in_double_colon_del_list(line, "abugida-default-vowel")
                        abugida_default_vowels = (
                            regex.split(r"[,;]\s*", abugida_default_vowel_s) if abugida_default_vowel_s else []
                        )
    ...
    def load_unicode_data_props(self, filename: str, load_log: bool = True):
        ...
        if load_log:
            ...
            if n_script_vowel_sign or n_script_virama:
                sys.stderr.write(
                    f", with a total of {n_script_vowel_sign} vowel signs " f"and {n_script_virama} viramas"
                )
            sys.stderr.write(".\n")
    ...
    def rebuild_unicode_data_props(self, out_filename: str, cjk: str = None, hangul: str = None):
        ...
        while codepoint < 0xF0000:
            ...
            for prop_name_comp2 in (
                ...
                for prop_name_comp in prop_list:
                    ...
                    script_name_cand = regex.sub(rf"\s+{prop_name_comp}\b.*$", "", char_name)
        ...
        sys.stderr.write(
            f"Rebuilt {out_filenames} with {n_script_refs} characters " f"for {len(d['script-names'])} scripts.\n"
        )
    ...
    def load_resource_files(self, data_dir: Path, load_log: bool = False, rebuild_ud_props: bool = False):
        ...
        if not isinstance(data_dir, pathlib.Path):
            ...
            return
        self.load_rom_file(
            os.path.join(data_dir, "romanization-auto-table.txt"), "ud", file_format="rom", load_log=load_log
        )
        self.load_rom_file(
            os.path.join(data_dir, "UnicodeDataOverwrite.txt"), "ow", file_format="u2r", load_log=load_log
        )
        self.load_rom_file(
            os.path.join(data_dir, "romanization-table.txt"), "man", file_format="rom", load_log=load_log
        )
    ...
    def apply_any_offset_to_cached_rom_result(
        cached_rom_result: Union[str, List[Edge]], offset: int = 0
    ) -> Union[str, List[Edge]]:
    ...
    def romanize_string_core(
        self, s: str, lcode: Optional[str], rom_format: RomFormat, cache_p: bool, offset: int = 0, **args
    ) -> Union[str, List[Edge]]:
    ...
    def romanize_string(
        self, s: str, lcode: Optional[str] = None, rom_format: RomFormat = RomFormat.STR, **args
    ) -> Union[str, List[Edge]]:
...
class Lattice:
    ...
    def expand_rom_with_special_chars(
        self, rom: str, start: int, end: int, **_args
    ...
    def add_default_abugida_vowel(self, rom: str, start: int, end: int, annotation: str = "") -> str:
        ...
        try:
            ...
            if self.uroman.dict_bool[("is-virama", next_s_char)]:
                return base_rom
            if self.uroman.char_is_nonspacing_mark(next_s_char) and self.uroman.dict_bool[("is-virama", next2_s_char)]:
                return base_rom
    ...
    def cand_is_valid(self, rom_rule: RomRule, start: int, end: int, rom: str) -> bool:
        ...
        if rom_rule["use_only_at_end_of_word"] and not self.is_at_end_of_word(end):
            return False
        if rom_rule["use_only_for_whole_word"] and not (
    ...
    def decomp_rom(self, char_position: int) -> Optional[str]:
        ...
        if ud_decomp_s := ud.decomposition(char):
            ...
            if (
                format_comps
                and (format_comps[0] not in ("<super>", "<sub>", "<noBreak>", "<compat>"))
                # and (char not in '')
                and (not other_comps)
                and decomp_s
            ):
        ...
        for start in range(self.max_vertex):
            for end in range(start + 1, self.max_vertex + 1):
                ...
                if (rom := self.simple_top_romanization_candidate_for_span(start, end)) is not None:
                    ...
                    rom, start2, end2, exp_edge_annotation = self.expand_rom_with_special_chars(
                        rom, start, end, annotation=edge_annotation, recursive=args.get("recursive", False)
                    )
    ...
    def find_rom_edge_path_backwards(
        self, start: int, end: int, min_char: Optional[int] = None, return_str: bool = False
    ) -> Union[List[Edge], str]:
...
def main():
    ...
    parser.add_argument("-l", "--lcode", type=str, default=None, help="ISO 639-3 language code, e.g. eng")
...
uroman.py ⴰⵣⵓⵍ -i ../test/multi-script.txt > ../test/multi-script-out2.txt
    ...
    if args.ignore_args:
        ...
        uroman.romanize_file(input_filename="../test/multi-script.txt", output_filename="../test/multi-script-out3.txt")
    else:
        ...
        if (
            ...
            uroman.romanize_file(
                args.input_filename, args.output_filename, lcode=args.lcode, direct_input=args.direct_input, **args_dict
            )
        if args.test:
